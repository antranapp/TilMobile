...
class _SettingGitSetupScreen extends State<SettingGitSetupScreen> {
            ...
            case 3: {
                if (_pageChoice[1] == SetupTypeChoice.Manual) {
                    return GitSshKeySelectionPage(
                        ...
                        onReuseGeneratedKeys: () {
                            setState(() {
                                _keyGenerationChoice = KeyGenerationChoice.AutoGenerated;
                                _pageCount = position + 2;
                                _reuseExistingKeys(context);
                            });
                        },
                        onUserProvidedKeys: () {
                            setState(() {
    ...
    void _startGitClone(BuildContext context) async {
        ...
        String repoPath = p.join(basePath, appState.localGitRemoteFolderName);
        String error;
        try {
    ...
    void _reuseExistingKeys(BuildContext context) {
        getSSHPublicKey().then((String publicKey) {
            setState(() {
                this.publicKey = publicKey;
                Log.d("PublicKey: " + publicKey);
                _copyKeyToClipboard(context);
            ...
            showSnackbar(context, "No public key found. Please generate or provide a new key pair");
        });
    }
    ...
    void _launchDeployKeyPage() async {
        ...
        var repoName = _gitCloneUrl.substring(_gitCloneUrl.lastIndexOf(":") + 1, lastIndex);
